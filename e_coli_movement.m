...
if ~isempty(higher_concentration_distances)
    disp(['Absolute Distance Covered by E. coli to Reach Higher Concentration: ' num2str(higher_concentration_distances(1) * 1e6) ' µm']);
...
ylabel('Time (s)');
colorbar;
caxis([min(sol(:)) max(sol(:))]); % Set color limits to concentration range
colormap('jet');
